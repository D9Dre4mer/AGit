...
class AGitApp:
    ...
    def _load_logo_assets(self):
        ...
        try:
            ...
            self._logo_ui = (
                base.subsample(ui_factor, ui_factor) if ui_factor > 1 else base
            )
            self._logo_icon = (
                base.subsample(icon_factor, icon_factor) if icon_factor > 1 else base
            )
    ...
    def create_widgets(self):
        ...
        self.repo_entry.pack(
            side=tk.LEFT, fill=tk.BOTH, expand=True, ipady=8, padx=(0, 10)
        )
    ...
    def reset_state(self):
        ...
        self.commit_button.config(
            text="Generate Commit Message", bg=COLORS["bg_button"]
        )
    ...
    def generate_message_process(self):
        ...
        try:
            ...
            self.update_status(
                "Generating commit message with AI...", COLORS["warning"]
            )
            ...
            self.root.after(
                0,
                lambda: self.commit_button.config(text="Commit", bg=COLORS["success"]),
            )
            self.root.after(0, lambda: self.cancel_button.config(state=tk.NORMAL))

            self.update_status(
                "Commit message generated. Review and click 'Commit' to proceed.",
                COLORS["success"],
            )
            self.set_button_state(True)
